...
// Get database schema
app.get('/api/database/schema', requireDatabase, async (req: express.Request, res: express.Response) => {
  try {
    const { database, client } = await import('./config/database');
    ...
    const schema = {
      ...
      collections: await Promise.all(collections.map(async (col) => {
        ...
        const docCount = await collectionRef.countDocuments();
        ...
        return {
          ...
          indexes: await collectionRef.indexes()
        };
      }))
    };
...
app.get('/api/database/collections', requireDatabase, async (req: express.Request, res: express.Response) => {
  try {
...
app.get('/api/database/collections/:collectionName/data', requireDatabase, async (req: express.Request, res: express.Response) => {
  try {
    ...
    const total = await collectionRef.countDocuments(filterObj);
...
app.post('/api/database/collections/:collectionName/query', requireDatabase, async (req: express.Request, res: express.Response) => {
  try {
    const { collectionName } = req.params;
    ...
    const total = await collectionRef.countDocuments(query);
